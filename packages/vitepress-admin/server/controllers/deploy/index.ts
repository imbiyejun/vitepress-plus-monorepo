...
// SSH connection config for test (without remotePath requirement)
function getConnectionConfig(): Omit<DeployConfig, 'remotePath'> | null {
...
export const testConnection = async (_req: Request, res: Response): Promise<void> => {
  ...
  if (!config) {
    sendError(
      res,
      '服务器连接配置未设置，请在 .env 文件中配置 DEPLOY_HOST, DEPLOY_USERNAME, DEPLOY_PASSWORD',
      400
    )
    return
  }
