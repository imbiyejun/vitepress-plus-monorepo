...
// Fetch directory contents with search support
const fetchDirectoryContents = async (dirPath?: string, search?: string) => {
  loading.value = true
  try {
    const data: DirectoryContents = await getDirectoryContents(dirPath, search || '')
    ...
    const errorMessage =
      ...
      (error instanceof Error ? error.message : '获取目录内容失败')
...
const handleSearch = async (keyword: string) => {
  ...
  try {
    await fetchDirectoryContents(currentPath.value, keyword)
  } catch (error) {
...
const handleRefresh = async () => {
  ...
  try {
    await fetchDirectoryContents(currentPath.value, searchKeyword.value)
  } finally {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    await Promise.all(selectedImages.value.map(imagePath => deleteImage(imagePath)))
...
const handleDownload = async (item: DirectoryItem) => {
  if (item.type === 'file') {
    await downloadImage(item.path, item.name)
  }
}
...
const handleRenameSubmit = async (newName: string) => {
  ...
  try {
    ...
    await renameImage(currentRenameImage.value.path, newName)
...
const handleCreateDirectorySuccess = async (directoryName: string) => {
  try {
    loading.value = true
    await createDirectory(currentPath.value || '', directoryName)
...
const handleDeleteDirectory = async (item: DirectoryItem) => {
  ...
  try {
    ...
    // Calculate directory path relative to public
